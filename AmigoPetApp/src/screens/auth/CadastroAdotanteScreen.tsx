...
export function CadastroAdotanteScreen() {
  ...
  const { control, handleSubmit, setValue, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      nome: '', email: '', senha: '', confirmar_senha: '', cpf: '', data_nascimento: '', telefone_1: '',
      telefone_2: '', cep: '', logradouro: '', numero: '', bairro: '',
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.sub}>Preencha seus dados para começar a adotar.</Text>
            ...
            <Controller control={control} name="cep"
              render={({ field: { onChange, value } }) => (
                <View>
                  <MaskInput
                    value={value}
                    onChangeText={(masked) => { onChange(masked); buscarCep(masked); }}
                    mask={CEP_MASK}
                    style={styles.maskInput}
                    keyboardType="numeric"
                  />
                  {buscandoCep && (
                    <Text style={styles.cepBuscando}>Buscando endereço...</Text>
                  )}
                </View>
              )} />
          </Campo>
...
const styles = StyleSheet.create({
  ...
  cepBuscando: { fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.xs, color: colors.secondary, marginTop: 2 },
