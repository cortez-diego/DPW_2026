...
export function CadastroClinicaScreen() {
  ...
  async function onSubmit(data: FormData) {
    setEnviando(true);
    try {
      await clinicaService.cadastrar({
        nome: data.nome,
      ...
      Alert.alert('Clínica cadastrada!', 'A clínica foi registrada com sucesso.', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (err: any) {
      const msg = err?.response?.data?.mensagem ?? err?.message ?? 'Erro ao cadastrar clínica.';
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <Text style={styles.heading}>Cadastro de Clínica</Text>
        <Text style={styles.sub}>Registre sua clínica veterinária na plataforma.</Text>

        <Secao titulo="Dados da clínica">
          <Campo label="Nome da clínica" error={errors.nome?.message}>
        ...
        >
          {enviando
            ...
            : <Text style={styles.btnLabel}>Cadastrar clínica</Text>}
        </TouchableOpacity>

        <TouchableOpacity style={styles.btnPular} onPress={() => navigation.goBack()} activeOpacity={0.7}>
          <Text style={styles.btnPularLabel}>Pular por agora</Text>
        </TouchableOpacity>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  btnPular: { alignItems: 'center', paddingVertical: spacing.md },
  btnPularLabel: { fontFamily: typography.fontFamily.body, fontSize: typography.fontSize.sm, color: colors.secondary },
});
