...
import { senhaSchemaZod, cpfValido } from '../../utils/validators';
...
export function CadastroVeterinarioScreen() {
  const { cadastrarVeterinario } = useAuth();
  ...
  const { control, handleSubmit, setValue, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      nome: '', cpf: '', crmv: '', email: '', senha: '', confirmar_senha: '',
      data_nascimento: '', telefone_1: '', telefone_2: '', cep: '',
      logradouro: '', numero: '', bairro: '', complemento: '', cidade: '', estado: '',
    },
  });
  ...
  return (
    <View style={styles.root}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <Text style={styles.heading}>Cadastro de Veterinário</Text>
        <Text style={styles.sub}>Registre-se para acompanhar a saúde dos animais.</Text>
        ...
        <Secao titulo="Dados profissionais">
          ...
          <Campo label="CRMV (ex: SP-12345)" error={errors.crmv?.message}>
            <Controller control={control} name="crmv"
              render={({ field: { onChange, onBlur, value } }) => (
                <TextInput mode="outlined" value={value} onChangeText={onChange} onBlur={onBlur}
                  autoCapitalize="characters" outlineColor={colors.border} activeOutlineColor={colors.primary}
                  style={styles.input} contentStyle={styles.inputContent} />
              )} />
          </Campo>
        ...
        <TouchableOpacity
          style={styles.btnClinica}
          onPress={() => navigation.navigate('CadastroClinica')}
          activeOpacity={0.8}
        >
          <Text style={styles.btnClinicaLabel}>Cadastrar clínica veterinária (opcional)</Text>
        </TouchableOpacity>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  btnClinicaLabel: { fontFamily: typography.fontFamily.bodyBold, fontSize: typography.fontSize.sm, color: colors.primary },
});
